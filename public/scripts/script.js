...
function chooseOption(opt) {
    ...
    } else if(currentOption == dialogue.prologue.options["n"]) { roomtraverse("trunk"); }
    ...
    if(currentOption == dialogue.trunk.options["c"]) { screamCount = screamCount + 1; sfhcount(); return false; }
    ...
    if(currentOption == dialogue.trunk_open.options["b"]) { screamCount = screamCount + 1; sfhcount(); return false; }
    ...
    if(currentOption == dialogue.trunk_out.options["c"]) { screamCount = screamCount + 1; sfhcount(); return false; }
    ...
    if(currentOption == dialogue.checkvehicle.options["a"] && (invTireIron == 1 && investigateVehicle == 0 && breakGlass == 0)) {
        clear()
        game.innerHTML = "<p>Clutching the tire iron in your white-knuckled fist, you strike the window once. The tire iron bounces off with little more than a loud pop. You strike it again, this time more towards \
    ...
    if(currentOption == dialogue.checkvehicle.options["b"]) { screamCount = screamCount + 1; sfhcount(); return false; }

    if(currentOption == dialogue.checkvehicle.options["c"] && breakGlass == 1) {
        clear()
        game.innerHTML = "<p>Now that you have broken the window, you are quite certain that you'll be able to get inside to thoroughly examine the vehicle. What you can see already from the shattered opening is a key hanging from the rear-view mirror. The possible \
    ...
    if(currentOption == dialogue.checkinterior.options["b"] && drankPunch == 1) {
        clear()
        game.innerHTML = "<p>The glove box is unlocked. You grab the handle and pull to release. The glove compartment drops open and breaks off the hinges, which sends various documents flying onto the pitch black floorboard.</p><p><i>(Doubt there's anything important. Just papers.)</i></p>";
        lookedForGloves = 1;
        }
    ...
    if(currentOption == dialogue.at_bridge.options["b"]) { screamCount = screamCount + 1; sfhcount(); return false; }
    ...
    if(currentOption == dialogue.on_bridge.options["b"] && (drinkRiver == 0 && lookedForGloves == 1)) {
        ...
        } else if(currentOption == dialogue.on_bridge.options["b"] && (drinkRiver == 1 || lookedForGloves == 0)) {
    ...
    if(currentOption == dialogue.lonewolf.options["a"] && (drinkRiver == 1 || invTalisman == 1)) {
        // Rule of Cool
        clear()
        game.innerHTML = "<p>You let out a scream to the high heavens, one which faintly echoes back. In the distance, somewhere far behind you, you hear a respondent howl. This, much to your delight, is enough to stir \
    ...
    if(currentOption == dialogue.lonewolf.options["b"] && (drinkRiver == 1 || invTalisman == 1)) {
        clear()
        game.innerHTML = "<p>Your fight or flight senses are raging, but you see clarity now. One foot after the other, you race towards the light, only occasionally looking back to see if you’re being followed.</p><p>Fortune \
        ...
        bearing down on you, a prime predator in its peak environment who’s spotted wounded prey ripe for the killing. It leaps and snaps at your ankle.</p><p>White hot pain rushes from your extremity and you drop to the ground. Your \
        hands move frantically across the earth in the desperate hopes of finding something with which to protect yourself, but there is nothing here for you.</p><p>The creature comes at you again. This time, it goes for the throat.</p> \
        <p>And it does not miss or waver in savagery.</p><p>As you feel the life leaving you, as your eyes begin to set in their final resting position, you swear you see a silhouette standing just off in your periphery.</p>";
        currentLocation = "gameover";
        }

    if(currentOption == dialogue.lonewolf.options["c"] && invTireIron == 1) {
        clear()
        game.innerHTML = "<p>Your fight or flight senses are raging, but you see clarity now. You can intimidate this beast. You just have to convince him that you aren’t one to be trifled with.</p><p>With the tire iron in hand, you \
        ...
        // GAME OVER
        clear()
        game.innerHTML = "<p>Your fight or flight senses are raging, but you see clarity now. You can intimidate this beast. You just have to convince him that you aren’t one to be trifled with.</p> \
...
// House - Back Porch
    if(currentOption == dialogue.house_bporch.options["a"] && invKey == 1) {
        clear()
        game.innerHTML = "<p>You remember the key you picked up from the car and realize that both the deadbolt and the key have the same logo, so what is there to lose? You tear the key off your neck with a firm yank and immediately \
    ...
    if(currentOption == dialogue.house_bporch.options["c"]) { screamCount = screamCount + 1; sfhcount(); return false; }


// Loose Soil/Tight Surprise
    if(currentOption == dialogue.loose_soil.options["y"] && (invTireIron == 1 && investigateBackdoor == 1)) {
        clear()
        game.innerHTML = "<p>You take the bladed end of the tire iron, stab it into the soil, and then wrench it to the side to further break loose the dirt that has had some time to pack.</p><p>The amount of effort expended, you \
...
// House - Side
    if(currentOption == dialogue.house_side.options["a"] && (lightsOn == 1 || invFlashlight == 1 && invKey == 1)) {
        clear()
        game.innerHTML = "<p>Now that you have illumination, sticking the key into the deadbolt is the easy part. Just like with the back door, you turn the key and the door is now unlocked.</p><p>You walk into the entryway and \
...
// House Interior - Kitchen
    ...
    if(currentOption == dialogue.house_kitchen.options["c"] && (investigatePhone == 0 && playerKitchen == 1)) {
        clear()
        investigatePhone = 1;
        game.innerHTML = "<p>It’s a wonder you didn’t trip and fall with the way you double-timed it over to the point. You lift the handset off the receiver hard enough that the bell inside resounds for a second or two.</p><p>Even \
...
function sfhcount() {
    ...
    if (random <= screamCount) {
        currentLocation = "gameover";
        clear()
        game.innerHTML = "<p>The silence after the end of your scream is only momentary.</p><p>Immediately following that, your ears are drowned in a deluge of noise as your surroundings melt like a surrealistic \
        ...
        game.innerHTML = "<p>You hear no response from the Ether.</p>";
        }
}
